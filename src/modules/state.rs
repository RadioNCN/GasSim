...
impl GasState {
    ...
    pub fn cv(&self) -> HeatCapacity {
        let ncv_l_H2: HeatCapacity =
            self.H2 * (self.cp_mole(self.T, FitConst::fit_nist_H2()) - NatConst::R());
        let ncv_l_H2O: HeatCapacity =
            self.H2O * (self.cp_mole(self.T, FitConst::fit_nist_H2O()) - NatConst::R());
        let ncv_l_O2: HeatCapacity =
            self.O2 * (self.cp_mole(self.T, FitConst::fit_nist_O2()) - NatConst::R());
        let ncv_l_N2: HeatCapacity =
            self.N2 * (self.cp_mole(self.T, FitConst::fit_nist_N2()) - NatConst::R());
    ...
    fn vis_dyn_part(&self, n: AmountOfSubstance, M: MolarMass, d: Length) -> DynamicViscosity {
        ...
        let roh: MassDensity = if V.value > 0. {
            m / V
        } else {
            MassDensity::new::<kilogram_per_cubic_meter>(0.)
        };
    ...
    fn vis_dyn_fit(&self, fit: [f64; 5]) -> DynamicViscosity {
        let T = self.T.value;
        let mut sum = 0.;
        fit.iter().enumerate().for_each(|(n, a)| {
            sum += a * T.ln().powi(n as i32);
        });
        sum *= T.powf(0.25);
        let mu = sum * sum;
    ...
    fn vis_dyn_suth(&self, mu0: DynamicViscosity, nist_fit: [f64; 8]) -> DynamicViscosity {
        ...
        mu0 * (self.T / NatConst::T_25()).value.powf(3. / 2.) * (NatConst::T_25().value + S0)
            / (self.T.value + S0)
    }
    ...
    fn wilke_phi(
        &self,
        mu_a: DynamicViscosity,
        mu_b: DynamicViscosity,
        M_a: MolarMass,
        M_b: MolarMass,
    ) -> f64 {
        if mu_b.is_normal() {
            1. / 8f64.sqrt()
                * (1. + (mu_a / mu_b).value.sqrt() * (M_b / M_a).value.powf(0.25)).powi(2)
                / (1. + (M_a / M_b).value).sqrt()
        } else {
            0.
        }
    }
    ...
    fn S0_part(&self, fit: [f64; 8], T: ThermodynamicTemperature) -> MolarHeatCapacity {
        ...
        let S0 = fit[0] * T.ln() + fit[1] * T + fit[2] * T.powi(2) / 2. + fit[3] * T.powi(3) / 3.
            - fit[4] / (2. * T.powi(2))
            + fit[6];
...
impl Add for GasState {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        let (dt, H2, H2O, O2, N2) = if self.dt.value > rhs.dt.value {
            (
                self.dt,
                (nH2l + nH2r) * self.dt,
                (nH2Ol + nH2Or) * self.dt,
