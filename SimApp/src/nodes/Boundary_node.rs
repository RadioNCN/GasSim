...
impl GasNode {
    ...
    pub fn show_input(&self, pin: &InPin, ui: &mut Ui, snarl: &Snarl<Node>) -> PinInfo {
        match &snarl[pin.id.node] {
            Node::Gas(GasNode::Boundary(GS, _)) => {
                let input = pin.remotes.last().and_then(|remote| BoundaryType::value_of_outpin(&*snarl, *remote));
                match input {
                    Some(GS_US) => {
    ...
    pub fn show_output(&mut self) -> PinInfo {
        match &self {
    ...
    pub(crate) fn show_body(&mut self, node: NodeId, inputs: &[InPin], outputs: &[OutPin], ui: &mut Ui) {
        match self {
            GasNode::Boundary(GS, n) => {
...
impl GasNode {
    ...
    pub fn show_state(ui: &mut egui::Ui, gs: &GasState){
        let mut gsm = gs.to_mass_rate();
        ui.with_layout(egui::Layout::top_down(egui::Align::LEFT),
            |ui| {
               let p = gsm.p.get::<bar>();
               ui.horizontal(|ui| {
